...
def select_hexes_by_random(hexes_by_label, current_round):
    ...
    if current_round == 1:
        ...
        controlled_print(current_round)
    return selected_hexes
...
def process_manual_input(input_str):
    ...
    try:
        ...
        print(current_turn,selected_coords)
...
def main(black_player, white_player, manual):
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if not turn_ended and ((current_turn == 'black' and black_player_type == "mcts") or \
                                   (current_turn == 'white' and white_player_type == "mcts")):
                ...
                selected_hexes = select_hexes_by_mcts(hexagon_board, current_round, current_turn)
...
if __name__ == "__main__":
    #########################################################################
    # Choose "human" for the human player and "mcts" for the MCTS Player
    #########################################################################
    if len(sys.argv) != 4:
        print("Usage: python main.py [player1_type] [player2_type] [manual]")
        print("player1_type and player2_type should be 'human' or 'mcts'")
        sys.exit(1)  # Exit the script with an error code
 
    main(sys.argv[1], sys.argv[2], sys.argv[3])

    # write in Terminal: python hw2_mcts_312551817.py human mcts True
